...
public class CustomCookingSerializer<T extends AbstractCookingRecipe> extends net.minecraftforge.registries.ForgeRegistryEntry<RecipeSerializer<?>> implements RecipeSerializer<T> {
    private final int defaultCookingTime;
    private final CustomCookingSerializer.CookieBaker<T> factory;

    public CustomCookingSerializer(CustomCookingSerializer.CookieBaker<T> p_44330_, int p_44331_) {
    ...
    public void toNetwork(FriendlyByteBuf buf, T t) {
        buf.writeUtf(t.getGroup());
        t.getIngredients().get(0).toNetwork(buf);
        buf.writeItem(t.getResultItem());
        buf.writeFloat(t.getExperience());
        buf.writeVarInt(t.getCookingTime());
    }

    public static Object getPrivateFieldValue(Object object, String name) throws Exception{
        Field ingredient = null;
        ingredient = object.getClass().getDeclaredField(name);
        ingredient.setAccessible(true);
        return ingredient.get(object);
    }
